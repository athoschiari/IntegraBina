...
//-- 23/05/2016 ------------------------------
//-- Versão 2.0  Atualizado para -> XE .. Delphi10
//-------------------------------------------
...
uses
  ...
  IdComponent,IdUDPServer,IdGlobal, ShellAPI,
 ...
 type
  ...
  TOpenPort = procedure(Sender: TObject; PortLocal: integer) of object;
  TClosePort = procedure(Sender: TObject; PortLocal: integer) of object;
...
type
  TBraiDRepeaterXE = class(TComboBox)
  private
  ...
  const AStatus: TIdStatus; const AStatusText: String);
    ...
    procedure nmDataReceived(AThread: TIdUDPListenerThread;
  ...
  published
    ...
    property OnOpenPort: TOpenPort read fOpenPort write fOpenPort;
    property OnClosePort: TClosePort read fClosePort write fClosePort;
...
procedure Register;
begin
 RegisterComponents('Sibel', [TBraiDRepeaterXE]);
end;

{Autoriza o uso da porta no FireWall (XP) -------------------------------------}
procedure TBraiDRepeaterXE.UnLookFireWall(App,Port:string; InternetAcess: Boolean);
var
Reg : TRegistry;
ap, SubNet:string;
begin
...
procedure TBraiDRepeaterXE.ShowConfig(Device:integer);
begin
...
procedure TBraiDRepeaterXE.StartPlugIn(PathApp, Registry:string);
var
S, SS:string;
begin
 ...
 // ATENÇÃO !!!! DELPHI XE / D10
 // ShellExecute(Handle, nil, PChar(S), nil,Pchar(Registry), SW_SHOWNORMAL); // ShellExecute: Unit ShellAPI;
...
procedure TBraiDRepeaterXE.LookFireWall(App,Port:string);
var
Reg : TRegistry;
ap, SubNet:string;
begin
...
constructor TBraiDRepeaterXE.Create(AOwner: TComponent);
var
...
{--- Monitoraro IP Local ------------------------------------------------------------}
function TBraiDRepeaterXE.GetIPFromHost(var HostName, IPaddr, WSAErr: string): Boolean;
type
...
procedure TBraiDRepeaterXE.OnTimerMonitorIP(Sender: TObject);
var
Err: string;
begin
...
procedure TBraiDRepeaterXE.ViewIPLocal;
var
  t : TIdIPWatch;
begin
...
destructor TBraiDRepeaterXE.Destroy;
begin
  try
...
procedure TBraiDRepeaterXE.nmDataReceived(AThread: TIdUDPListenerThread;
      const AData: TIdBytes; ABinding: TIdSocketHandle);
var
  s: string[250];
  ss : String;
...
try
  ...
  try
   ...
   // KData.CopyFrom(BytesToString(AData), 0);
   ...
   // if numberbytes = MsgSize then
    ...
    //  kData.Read(s[1], MsgSize);
      ...
      begin
        ...
        if assigned(fcalled) then fcalled(Self,ss);
      end
    end;
  finally
   // kData.Clear;
...
/////
procedure TBraiDRepeaterXE.SetDDDLocal(const Value: string);
begin
 if(Value<>fDDD)then
     fDDD := Value;
end;
procedure TBraiDRepeaterXE.SetPathPlugIn(const Value: string);
begin
...
procedure TBraiDRepeaterXE.SetHostBraiD(const Value: Boolean);
begin
...
procedure TBraiDRepeaterXE.SetIP(const Value: string);
begin
...
procedure TBraiDRepeaterXE.SetActive(const Value: boolean);
begin
 if Value = True then
    fActive := True
 else fActive := FALSE;
...
procedure TBraiDRepeaterXE.SetPort(const Value: integer);
begin
  if assigned(UDPNM) then
  begin
    fport := Value;
...
procedure TBraiDRepeaterXE.SetnLine(const Value: integer);
begin
...
procedure TBraiDRepeaterXE.SetLocalPort(const Value: integer);
begin
  if(Value=fLocal)then exit;
  ...
  if assigned(UDPNM) then begin
    ...
    UDPNM.Free;
    //kData.Free;
    if assigned(fClosePort)then fClosePort(Self, Value);
  try
    UDPNM := TidUDPServer.Create(self);
    ...
    UDPNM.ThreadedEvent := True;
    UDPNM.Active := True;
    if assigned(fOpenPort)then fOpenPort(Self, Value);
...
procedure TBraiDRepeaterXE.nmStatus(ASender: TObject;
     const AStatus: TIdStatus; const AStatusText: String);
begin
...
procedure TBraiDRepeaterXE.ReceiveIP(var S: string);
var
  {a,} b: string;
  e, f: integer;
  g: string;
...
function TBraiDRepeaterXE.BraiDDisConnect(Line:Integer):Boolean;
begin
 if(not fHostBraiD)then begin
...
function TBraiDRepeaterXE.BraiDConnect(IP: string;Line:Integer):Boolean;
var
  i: integer;
  vIP, vPort: string;
  VP: boolean;
...
procedure TBraiDRepeaterXE.SetNetAcess(const Value: boolean);
begin
if(fNetAcess<>Value)then fNetAcess := Value;
end;

procedure TBraiDRepeaterXE.SetUnlookUDP(const Value: boolean);
begin
if(fUnlookUDP<>Value)then fUnlookUDP := Value;
end;

procedure TBraiDRepeaterXE.SetCaller(const Value: boolean);
var
  s: string;
  F : Byte;
  KK : String;
...
procedure TBraiDRepeaterXE.SendData(S: string);
var
 SS:String;
 F: byte;
 IPRemoto:string;
...
begin
 ...
 try
  if(fActive)and(UDPNM.Active)then  begin
    ...
    raise Exception.Create('Erro #3820 : TBraiDRepeaterXE.SendData()');
 end;
end;
procedure TBraiDRepeaterXE.OnTimerSendEvents(Sender: TObject);
var
y:integer;
begin
// fTimerSendEvents.Enabled:=FALSE;
 case fTipyEvent of
     ...
     7:  begin
          if assigned(fFormatPhone) then fFormatPhone(Self,fSendnLine,fSendText,TRUE,FALSE);
          if assigned(fsTatusEvent) then fsTatusEvent(Self,fSendnLine,'BREAK_FSK');
         end;
     8: begin
          if assigned(fFormatPhone) then fFormatPhone(Self,fSendnLine,fSendText,FALSE,TRUE);
          if assigned(fsTatusEvent) then fsTatusEvent(Self,fSendnLine,'BREAK_DTMF');
        end;
...
procedure TBraiDRepeaterXE.ReceiveData(S: string);
var
  P: string;
  i,y,p2: integer;
  rEnd: boolean;
